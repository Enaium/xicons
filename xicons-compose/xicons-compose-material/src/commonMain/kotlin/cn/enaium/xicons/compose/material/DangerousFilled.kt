// Generated by XIcons
// 2025-08-17 11:04:47
package cn.enaium.xicons.compose.material

import androidx.compose.ui.graphics.SolidColor
import androidx.compose.ui.graphics.vector.ImageVector
import androidx.compose.ui.graphics.vector.group
import androidx.compose.ui.graphics.vector.path
import androidx.compose.ui.unit.dp
import cn.enaium.xicons.compose.MaterialIcons

public val MaterialIcons.Filled.Dangerous: ImageVector
    get() {
        if (_Dangerous != null) {
            return _Dangerous!!
        }
        _Dangerous = ImageVector.Builder(
            name = "Dangerous",
            defaultWidth = 24.0.dp,
            defaultHeight = 24.0.dp,
            viewportWidth = 24.0f,
            viewportHeight = 24.0f
        ).apply {
            path(fill = SolidColor(androidx.compose.ui.graphics.Color.Black)) {
                moveTo(15.73f, 3.00f)
                horizontalLineTo(8.27f)
                lineTo(3.00f, 8.27f)
                verticalLineTo(15.73f)
                lineTo(8.27f, 21.00f)
                horizontalLineTo(15.73f)
                lineTo(21.00f, 15.73f)
                verticalLineTo(8.27f)
                lineTo(15.73f, 3.00f)
                close()
                moveTo(17.00f, 15.74f)
                lineTo(15.74f, 17.00f)
                lineTo(12.00f, 13.26f)
                lineTo(8.26f, 17.00f)
                lineTo(7.00f, 15.74f)
                lineTo(10.74f, 12.00f)
                lineTo(7.00f, 8.26f)
                lineTo(8.26f, 7.00f)
                lineTo(12.00f, 10.74f)
                lineTo(15.74f, 7.00f)
                lineTo(17.00f, 8.26f)
                lineTo(13.26f, 12.00f)
                lineTo(17.00f, 15.74f)
                close()
            }
        }.build()
        return _Dangerous!!
    }

private var _Dangerous: ImageVector? = null
